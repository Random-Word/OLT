...
$taskName = "operationalized_behaviour"; // this should be programmatically inserted
$taskNameForCss = "operationalized-behaviour"; // this should be programmatically inserted
...
$formId = ___strToSel(array($slide['name'], $taskName,'form'));
$controls_array = array( "buttons" => array( array( "action"  => "refresh",
                                             ...
                                             array( "action"  => "jumpList",
                                                    ...
                                                    "options" => array(
	                                                    ...
	                                                    "stop"     => $video_4['properties']['duration'], // time to stop incrementing
	                                                    "label"    => null )
...
$video_4['properties']['intervalize'] = $interval_value; // todo: this kind of thing should probably be done in the controller, eventually, by inferring the need from the schema
$video_4['properties']['on'] = 'e.intervalChange';
$video_4['properties']['do'] = 'local.loadInterval';

// build selectors for response ui
$counterId = ___strToSel( array( $slide[ 'name' ], $taskName, "counter" ));
$tallyId = ___strToSel( array( $slide[ 'name' ], $taskName, "tally" ));
$videoId = ___strToSel(array($slide[ "name" ], $taskName, "video"));
$finalizerId = ___strToSel( array( $slide[ 'name' ], $taskName, "finalizer" ));
$dataFixtureId = ___strToSel( array( $slide[ 'name' ], $taskName, "data fixture" ));
$intervalFixtureId = ___strToSel(array($slide['name'], $taskNameForCss, "interval-label", "fixture"));

// build interval strings
$intervalStrings = array();

for ($i=0; $i<ceil($video_4['properties']['duration'] / 10); $i++) {
...
<?php switch ($sectionName):
		case "operationalized_behaviour_counting":
							...
							<div class="small-12 columns">
								<ul class="olt-heading menu">
									<li class="heading togggler>">
										<a class="left-off-canvas-toggle" href="#" ><span class="small note-text">&lsaquo;&lsaquo;&nbsp;View Tally</span></a>
									</li>
								</ul>
							</div>
						</div>
							...
							<div class="row">
								<div class="small-12 columns">
									<div class="olt-datasheet dark compact read-only olt-response">
										<h4 class="text-center" ><?php echo $operationalized_behaviour['label'];?></h4>
										<dl id="<?php echo $dataFixtureId;?>" class="h5-sized">
											...
											<dt class="small-6 columns olt-rfield descriptor">
												...
												<span id="<?php echo ___strToSel(array($slide['name'], "$taskNameForCss-tally", $interval_str,"fixture"));?>"
												      class="value">0</span>
												<?php } ?>
											</dd>
										</dl>
								...
								<div class="olt-datasheet">
									<dl class="h3-sized">
										...
										><dd class="small-8 columns value text-center">
											<?php echo ucfirst( $operationalized_behaviour['label']); ?>
										</dd
										><dt class="small-4 columns descriptor">Description</dt
																				><dd class="small-8 columns value">
																					<span class="diminished"><?php echo $operationalized_behaviour['description']; ?></span>
																				</dd
										...
										<?php echo $this->Element("lab_ui".DS."counter", array(
												...
												"param_or_response_name" => $taskName,
												"incrementer" => true,
												"decrementer" => true,
												"triangles" => true,
		...
		<div class="row">
					<div class="small-12 columns">
						...
						echo $this->Element("lab_ui/save_button", array("text" => "Save",
						                                                      ...
						                                                      "write" => array(),
						                                                      "data"=> array("create-keys"=>true, "create-after-depth" => 2),
						                                                      "target"=> $formId));?>
					</div>
				</div>
	</div>
	<div id="rset-data">
		<ul class="pagevar menu" data-for-pagevar="operational_definition">
			<li <?php echo ___dA(array("pagevar-opt" => ___asParam($operationalized_behaviour['label']),
			                           "active" => true));
				?>>
				<h4 class="<?php if ($i == 0) echo "active";?>"><a href="#"> <?php echo $operationalized_behaviour['label'];?></a></h4>
			</li>
		</ul>
	<?php
	...
	foreach ($intervalStrings as $iStr) {
		echo $this->Form->input($i, array("type" => "hidden",
		                                "id" => ___strToSel(array($slide['name'], "$taskNameForCss tally", $operationalized_behaviour['label'],
		                                                                             $iStr,"value")),
		                                "class" => "hidden",
		                                "label" => false,
		                                "div" => false,
		                                "value" => 0,
		                                "name" =>"data[response][".$slide['name']."][$taskName][".$operationalized_behaviour['label']."][$iStr]",
		                                "data-initialized" => 1
		                                ));
	}
	echo $this->Form->end();
...
var local = {
		...
		fetchList:{
			player:"manifest.html5v.<?php echo ___strToSel(array($slide['name'], $taskName,"video"));?>",
		...
		generateSelector: function(intervalStr, operationalDefinition, fixture) {
			...
			var base = "<?php echo ___strToSel(array($slide['name'], "$taskNameForCss tally"));?>";
