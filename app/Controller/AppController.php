...
class AppController extends Controller {
	public $components = array( "Session", "Error", "RequestHandler", 'Acl',
	                            'Auth' => array(
		                            'authorize' => array(
	...
	public $helpers = array( "Session", "Html", "Form", "Lex" );
	...
	static function as_parameter($string) {
		// I use the same formatting conventions for files and inter-language params (js to php and back)
		// but using as_file_name() to achieve the same formatting outside the context of file access is confusing
		return AppController::as_file_name( $string );
	}
...
/* NOTES (i'm keeping them here b/c I dunno where else I won't forget to find them)
 * - create a helper, "OLTStrings" or something, at some point, that will allow users, when building a lab to decide
 *  to represent certain strings as manipulations thereof (such as "singular" or "plural", etc.); idea arose when
 *  realizing that the ethogram slides can't easily be called "cat videos" unless the entire param is renamed "cat"
 *
 *
 *
 *
